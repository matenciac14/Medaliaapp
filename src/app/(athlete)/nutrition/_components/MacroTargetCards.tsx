...
const MACROS = [
  { key: 'kcal',     label: 'Energía',   unit: 'kcal', color: 'text-[#ea580c]', bg: 'bg-orange-50', border: 'border-orange-100' },
  { key: 'proteinG', label: 'Proteína',  unit: 'g',    color: 'text-blue-600',  bg: 'bg-blue-50',   border: 'border-blue-100'   },
  { key: 'carbsG',   label: 'Carbos',    unit: 'g',    color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'border-yellow-100' },
  { key: 'fatG',     label: 'Grasas',    unit: 'g',    color: 'text-green-600', bg: 'bg-green-50',  border: 'border-green-100'  },
] as const
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        {MACROS.map(({ key, label, unit, color, bg, border }) => (
          <div key={key} className={`rounded-xl border px-3 py-2.5 ${bg} ${border}`}>
