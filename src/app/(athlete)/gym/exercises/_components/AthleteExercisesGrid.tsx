          ...
          return (
            ...
            >
              ...
              <div className="p-3 flex flex-col gap-2">
                ...
                <p className="text-[10px] text-gray-400">{ex.equipment}</p>
              </div>
            </button>
          )
        })}
...
function ExerciseModal({ exercise, onClose }: { exercise: Exercise; onClose: () => void }) {
  ...
  function Badge({ children, color }: { children: React.ReactNode; color: string }) {
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${BADGE[color] ?? BADGE.gray}`}>
        {children}
      </span>
    )
  }
      ...
      >
        {exercise.gif && (
          <div className="bg-gray-50">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={exercise.gif} alt={exercise.nameEs ?? exercise.name} className="w-full max-h-64 object-contain" />
          </div>
        )}
        ...
        <div className="p-5 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {exercise.nameEs && <p className="text-xs text-gray-400 mt-0.5">{exercise.name}</p>}
            </div>
          ...
          {exercise.secondaryMuscles.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Músculos secundarios</p>
              <div className="flex flex-wrap gap-1.5">
                {exercise.secondaryMuscles.map(m => <Badge key={m} color="gray">{m}</Badge>)}
              </div>
            </div>
          )}
          ...
          {(exercise.instructionsEs.length > 0 || exercise.instructions.length > 0) && (
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Instrucciones</p>
              <ol className="flex flex-col gap-2">
                {(exercise.instructionsEs.length > 0 ? exercise.instructionsEs : exercise.instructions).map((step, i) => (
                  <li key={i} className="flex gap-3 text-sm text-gray-700 leading-snug">
                    <span className="shrink-0 w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center text-white mt-0.5" style={{ backgroundColor: '#1e3a5f' }}>
                      {i + 1}
                    </span>
                    {step}
                  </li>
