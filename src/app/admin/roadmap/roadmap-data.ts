...
export const GROUPS: RoadmapGroup[] = [
...

  {
    ...
    items: [
      ...
      { title: 'INT-P0 — User creation no garantiza registros dependientes: UserSubscription y CoachProfile pueden no existir', done: false, priority: 'P0', note: 'Detectado en auditoría seed jul-2026. Registro de usuarios (web + mobile) no crea UserSubscription ni CoachProfile en la misma transacción. Cualquier código que lea user.subscription.tier o coach.coachProfile puede NPE. Fix: mover creación de UserSubscription (siempre) y CoachProfile (si role=COACH) al use case de registro dentro de $transaction. Aplica a /api/auth/register, /api/mobile/auth/register y /api/coach/clients/create.' },
      { title: 'INT-P0 — Auditoría exhaustiva DB + código: identificar todas las invariantes sin enforce en create use cases', done: false, priority: 'P0', note: 'Pendiente: análisis completo de los 25+ modelos del schema buscando: (1) registros dependientes sin creación atómica, (2) campos en sync que pueden desincronizarse, (3) estados inválidos permitidos por el schema que el código asume imposibles, (4) flujos de eliminación con cascade incorrecto. Entregable: script scripts/check-db-integrity.ts + fixes en use cases.' },
    ],
  },
