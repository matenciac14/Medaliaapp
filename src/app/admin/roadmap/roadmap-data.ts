...
export const GROUPS: RoadmapGroup[] = [
...

  {
    ...
    items: [
      ...
      { title: 'CQ-02 — map-athlete.ts importado desde API route — violación de capas', done: true, priority: 'P1', note: 'DONE: CoachAthleteRow + mapCoachAthleteRelation en infrastructure/db/coach-athlete.mapper.ts. _lib/map-athlete.ts es re-export shim.' },
      ...
      { title: 'CQ-07 — fmt() duplicada en coach/finanzas y admin/finanzas', done: true, priority: 'P2', note: 'DONE: extraída a src/lib/utils/format-currency.ts → formatCurrency(). Ambas pages importan con alias `as fmt` sin cambios en el resto del código.' },
      ...
      // P3
      { title: 'CQ-09 — TAKE = 20 — constante de paginación mezclada con mapper', done: true, priority: 'P3', note: 'DONE: TAKE definida como const local en athletes/page.tsx. Eliminada del shim _lib/map-athlete.ts.' },
      { title: 'CQ-10 — type InputRow — nombre ambiguo en map-athlete.ts', done: true, priority: 'P3', note: 'DONE: InputRow no tenía consumers externos. Alias eliminado del shim. Tipo canónico es CoachAthleteRow en infrastructure/db/coach-athlete.mapper.ts.' },
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            priority: 'P2',
            note: 'DONE: GET /api/coach/athlete/[id]/dailylogs — verifica coachRelation, devuelve últimos 7 días de DailyLog (peso, energía, FC, sueño). AthleteDetailClient: dailyLogs state + useEffect lazy en tab Resumen. Tabla con badge semafórico de energía (verde ≥4 / amarillo 3 / rojo <3).',
          },
          {
        ...
        items: [
          {
            ...
            priority: 'P1',
            note: 'Implementado. plans COACH → generateSuggestions() → PrismaSuggestionRepository.createMany() dentro del $transaction. Plans AI → auto-apply (comportamiento anterior intacto). applySessionAdjustments() exportada. generate-suggestions.ts en domain/check-in. Schema: SuggestionStatus/SuggestionType enums + CheckInSuggestion model. DB aplicada via prisma db push (2026-07-09). Tests: process-check-in.use-case.test.ts + generate-suggestions.test.ts — todos ✅.',
          },
          {
    ...
    phases: [
      {
        ...
        items: [
          ...
          { title: 'Dashboard coach: pagos vencidos visibles en dashboard principal', done: true, priority: 'P2', note: 'DONE: query Payment[PENDING, dueDate < now, coachId] en dashboard/page.tsx. Widget "Pagos vencidos" con borde rojo, badge conteo, días vencido, monto y CTA "Cobrar →" → /coach/athlete/[id]. Solo visible si hay pagos vencidos.' },
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'COACH-FEAT-01: Template selector en builder de planes — grid de 5 cards + plan en blanco', done: true, priority: 'P1', note: 'DONE: Builder sin plan muestra grid de 5 templates (5K, 10K, Media Maratón, Recomposición, Fuerza) + botón "Plan en blanco". Click template → form con nombre pre-llenado y semanas readonly. API POST /api/coach/athlete/[id]/plan/from-template crea plan con sessions pre-pobladas desde templates.ts usando $transaction con createMany. PlanBuilderClient.tsx: mode: initial|blank-form|template-form (reemplaza creating: boolean).' },
          ...
          { title: 'ARCH-06 — POST /api/coach/athlete/[id]/sessions — publicar sesión individual en el calendario del atleta', done: true, priority: 'P1', note: 'DONE: src/app/api/coach/athlete/[id]/sessions/route.ts. Body: { date: YYYY-MM-DD, type, durationMin, zoneTarget?, detailText?, coachNote? }. Busca PlanWeek activa que contenga la fecha (startDate ≤ date ≤ endDate, plan.status=ACTIVE). Deriva dayOfWeek desde Date.getUTCDay() (1=Lun...7=Dom). 422 si no hay plan activo para esa fecha. Zod validation. Útil para sesiones ad-hoc fuera del template de semanas.' },
        ],
      },
      {
        id: 'coach-avanzado',
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'NUT-05 — PlannedMeal del coach visible y unificado en mobile', done: true, priority: 'P1', note: 'DONE (2026-08-01). Verificado: no hay bug. Coach POST /api/coach/athlete/[id]/nutrition/plan escribe PlannedMeal con userId=athleteId (línea 103). Mobile GET /api/mobile/nutrition/plan lee PlannedMeal con where userId=mobile.id + fecha, incluyendo food con macros. La cascada PlannedMeal > AssignedNutritionPlan > tracking libre se resuelve client-side en mobile. Documentada con comentario en el endpoint mobile.' },
          ...
          { title: 'NUT-07 — Coach puede expandir template → PlannedMeal por semana desde tab atleta', done: true, priority: 'P1', note: 'DONE (2026-08-01). POST /api/coach/athlete/[id]/nutrition/apply-template con { weekStartDate }. Lee AssignedNutritionPlan + getIntensityMapForDateRange para mapear cada dia a HARD/EASY/REST. Transaction: deleteMany + createMany. Boton "Aplicar a esta semana" en tarjeta de template asignado (AthleteDetailClient). Re-mount CoachPlannedMealPlanner via key.' },
          ...
          { title: 'NUT-14 — Endpoint unificado mobile: GET /api/mobile/nutrition/today', done: true, priority: 'P2', note: 'DONE. GET /api/mobile/nutrition/today devuelve targets, intensity, plannedMeals, templateMeals, foodLogs, adherence, adjustment, water en un solo call. Endpoints legacy conservados.' },
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'Garantía con términos explícitos — "30 días o te ayudamos a exportar todo"', done: true, priority: 'P2', note: 'DONE: Badge verde "✓ 30 días de garantía" centrado encima del título de la sección garantía en landing.' },
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'GYM-MOB-04 — Sugerencia de próxima carga en sesión de gym mobile', done: false, priority: 'P2', note: 'El placeholder del input ya muestra el peso anterior y el delta vs sesión previa en verde/rojo. Falta: (1) agregar campo completed al map de previousLogs en /api/gym/session/today/route.ts (líneas 137-142 y 229-235); (2) lógica en gym-session.tsx: si todos los prev sets del ejercicio tienen completed:true → mostrar badge "→ Sugerido: Xkg" (+2.5kg) encima del campo de peso. El atleta puede ignorarlo y escribir cualquier valor. Sin schema changes. Sin automatización — solo es sugerencia visual. Fuente: análisis competitivo MyFitCoach 2026-08-05.' },
        ],
      },
    ],
  },
...

  {
    ...
    items: [
      ...
      // ── P1 — CHECKOUT Y WEBHOOK: primer cobro real ──────────────────────────
      {
        ...
        priority: 'P1',
        note: 'DONE. POST /api/billing/coach/checkout — auth COACH, valida upgrade al tier siguiente, llama createCoachCheckout(gateway). Arquitectura gateway-agnostic: IPaymentGateway port en domain/ports/payment-gateway.port.ts. StubPaymentGateway (dev) + factory getPaymentGateway() leen PAYMENT_GATEWAY env. Pendiente: UI UpgradeModal en el coach cuando recibe 402 COACH_LIMIT_REACHED.',
      },
      {
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'Coach ajusta targets de macros del atleta individualmente (sin tocar template base)', done: true, priority: 'P1', note: 'DONE: PATCH /api/coach/athlete/[id]/nutrition/targets. Verifica CoachAthlete ownership. Zod: { targetKcalHard?, targetKcalEasy?, targetKcalRest?, proteinG? }. Persiste en NutritionPlan.source="COACH". No toca template base del sistema.' },
          ...
          { title: 'COACH-NUT-UI-01 — KPIs banda top en Tab Nutrición coach (adherencia %, TDEE, proteína media, fase del plan)', done: false, priority: 'P2', note: 'Figma 4006:31 muestra 4 KPIs en banda horizontal: Adherencia semanal (% de 7 días), TDEE base (kcal/día), Proteína media (últimos 7 días), Fase del plan (DESARROLLO semana X de Y). Datos ya disponibles en NutritionPlan + FoodLog + TrainingPlan. Solo falta el componente visual.' },
          { title: 'COACH-NUT-UI-02 — Gráfica "Kcal consumidas vs target" en Tab Nutrición coach (barras 7 días)', done: false, priority: 'P2', note: 'Figma 4006:31 muestra bar chart con barras naranjas (consumido) vs línea gris (target) por día. Datos ya disponibles: FoodLog (consumido) + getDailyNutritionTarget por intensidad (target). Componente SVG/div nativo, misma UX que barras en atleta /progress.' },
          { title: 'COACH-NUT-UI-03 — Donut "Distribución de macros" en Tab Nutrición coach (promedio 7 días)', done: false, priority: 'P2', note: 'Figma 4006:31 muestra donut con kcal/día centro + barras de progreso proteína/carbs/grasa con % de cumplimiento. Datos: FoodLog 7 días promediados vs NutritionPlan targets. Componente donut SVG reutilizable.' },
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'NUT-04 — ProposeFoodUseCase + IFoodProposalRepository + PrismaFoodProposalRepository', done: true, priority: 'P1', note: 'DONE: domain/ports/food-proposal.repository.ts (IFoodProposalRepository interface: propose, listByUser, listPending, review, countPending). infrastructure/db/food-proposal.repository.ts (PrismaFoodProposalRepository): propose=$transaction(Food.create{source:community,isVerified:false}+FoodProposal.create), review=$transaction(FoodProposal.update+Food.update{isVerified:true}|{isActive:false}).' },
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'Progresión automática de cargas ajustada: incremento si >90% sets completados, baja si <60%', done: true, priority: 'P2', note: 'DONE: computeProgressionUpdates en domain/gym/complete-gym-session.use-case.ts — ratio>0.9 → +2.5kg, ratio<0.6 → −2.5kg (floor 0), zona gris → sin cambio. Constantes PROGRESSION_UP/DOWN_THRESHOLD + PROGRESSION_STEP_KG. 5 nuevos tests.' },
    ...
    phases: [
      ...
      // ── PERFORMANCE & FAT ROUTES — Auditoría 2026-07-03 ─────────────────────
      {
        ...
        items: [
          ...
          { title: 'PERF-12 — getDailyNutritionTargets en athlete-formulas.ts: posible dead code vs getDailyNutritionTarget de daily-target.ts', done: true, priority: 'P3', note: 'DONE: lib/core/athlete-formulas.ts — función getDailyNutritionTargets y tipo DayLoad eliminados (dead code confirmado, 0 callers fuera del test). lib/core/athlete-formulas.test.ts — todos los tests de getDailyNutritionTargets eliminados. Solo quedan tests de getAccountStatus.' },
      ...
      // ── Auditoría UI Módulos Atleta B2C — 2026-07 ────────────────────────────
      {
        ...
        items: [
          ...
          },
          {
            ...
            priority: 'P1',
            note: 'DONE (fix intermedio): plan/page.tsx — detecta sport=RUNNING via healthProfile y muestra landing diferenciada: ícono 🏃, título "Listo para tu plan de running", 3 feature cards (periodización, zonas Karvonen, check-in adaptativo), CTA "Activar mi plan de running". Atleta GYM-only sin plan sigue viendo estado genérico. Fix definitivo pendiente: AI genera plan B2C cuando se integre.',
          },
          {
  ...
  // ── ARQUITECTURA DE PLANES — PIVOT PRODUCTO ────────────────────────────────
  {
    ...
    phases: [
      ...
      },
      {
        ...
        period: 'BLOQUEADO — Requiere integración AI (sin fecha)',
        items: [
          {
            title: 'PLAN-B-01 — Use case: generatePlanForAthlete() con PlanSource.ATHLETE',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Decisión de producto 2026-07-09: el sistema no genera planes para atletas. Los planes vienen del coach (B2B) o de AI cuando se integre. Retomar cuando AI-Coach esté activo.',
          },
          {
            title: 'PLAN-B-02 — POST /api/athlete/plan/build — endpoint plan builder B2C',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Depende de PLAN-B-01. Sin generación automática de planes por decisión de producto.',
          },
          {
            title: 'PLAN-B-03 — GET /api/athlete/plan/templates — templates disponibles para B2C',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Sin UI de selección de templates hasta activar plan builder.',
          },
          {
            title: 'PLAN-B-04 — Cancelar plan propio: DELETE /api/athlete/plan/[planId]',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Solo aplica cuando atleta pueda crear planes propios.',
          },
          {
            title: 'PLAN-B-05 — Auto-completar plan self-built al llegar a semana N+1',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Depende de PLAN-B-01.',
          },
          {
            title: 'PLAN-F-01 — Wizard UI 3 pasos: objetivo → fecha inicio → confirmar',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Sin generación de planes no hay wizard. Retomar con AI.',
          },
          {
            title: 'PLAN-F-02 — Preview del template antes de confirmar en wizard',
            done: false,
            priority: 'P3',
            note: '[BLOQUEADO] Depende de PLAN-F-01.',
          },
          {
          ...
          },
          {
            ...
            priority: 'P3',
            note: '[BLOQUEADO] Solo aplica con planes ATHLETE source.',
          },
        ],
      },
    ],
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            priority: 'P1',
            note: 'DONE: migración 20260712000001_db06_plansource_athlete. ALTER TYPE "PlanSource" ADD VALUE "ATHLETE". Enum ahora: AI|COACH|AI_COACH_APPROVED|ATHLETE. Desbloquea PLAN-B-01 (plan builder autónomo B2C). Schema.prisma actualizado.',
          },
          {
    ...
    phases: [
      ...
      // ── Coach ────────────────────────────────────────────────────────────────
      {
        ...
        items: [
          ...
          {
            title: 'UX-COACH-05 — Consolidar 9 tabs a 6 en /coach/athlete/[id]: Ejercicios+Sesiones+Adherencia→Gym, Benchmarks→Progreso',
            done: false,
            priority: 'P1',
            note: 'TABS de 9 a 6: Resumen · Plan · Progreso · Nutrición · Gym · Mensajes. Progreso absorbe Adherencia (diet) + Benchmarks. Gym unifica Ejercicios (rutina asignada) + Sesiones (logs, PRs, curvas 1RM) + Adherencia gym (volumen, semáforos). Cambios: TABS const, useEffect triggers (Ejercicios/Sesiones/Adherencia → Gym), JSX conditionals. Figma ya tiene los frames fusionados como referencia.',
          },
        ],
      },
