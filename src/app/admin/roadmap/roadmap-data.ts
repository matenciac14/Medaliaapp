    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'Dashboard FREE — pantalla de bienvenida dedicada para atleta sin plan', done: false, priority: 'P1', note: 'Hoy el dashboard en modo FREE muestra cards vacías + CTA pequeño ("Comenzar →" → /new-goal) que debe eliminarse con ARCH-01. El atleta nuevo ve una app rota. Reemplazar por pantalla de bienvenida con 2 opciones claras: (1) "Busca un entrenador" → /coaches o WhatsApp, (2) "Entrena libre" → empieza a registrar sesiones sin plan. Sin planes auto-generados — el plan lo da el coach o el AI-Coach (roadmap). El atleta que registra sesiones libres ya tiene valor del producto desde el día 1. buildFreeModeSummary() ya existe — el problema es de prominencia y diseño, no de lógica.' },
          { title: 'Pantalla de celebración al completar plan + elección de siguiente paso', done: false, priority: 'P1', note: 'Hoy cuando el plan expira → status COMPLETED → banner pequeño "¿Lista tu próxima meta?". El atleta termina 12 semanas y no hay celebración. Implementar: pantalla de celebración con logro (nombre del plan, X/Y sesiones completadas, Z semanas). Dos opciones: (1) "Quiero otro plan" → B2B: contacta a su coach. B2C: modo FREE con CTA "Busca un entrenador" → /coaches. (2) "Seguir entrenando libre" → dashboard FREE. NO redirigir a /new-goal — ARCH-01 depreca la generación automática de planes. lastCompletedPlanInfo ya captura sessionsLogged/sessionsTotal — solo falta la UI.' },
        ],
      },
      {
        id: 'atleta-nutricion',
        ...
        items: [
          ...
          { title: 'Ajuste nutricional por intensidad real: notificación + aceptar/rechazar', done: false, priority: 'P1', note: 'Domain + API completos. calcNutritionAdjustment() pura, PendingNutritionAdjustment en schema, endpoints /api/mobile/nutrition/adjustment/[id]/accept|reject existen, GET /api/mobile/nutrition devuelve pendingAdjustment, BUG-038 (web): endpoints web + NutritionAdjustmentCard ya implementados. PENDIENTE: UI mobile — la pantalla de nutrición en mobile no muestra la card del ajuste pendiente ni llama a los endpoints. Atleta mobile nunca ve el ajuste.' },
      ...
      },
      {
        ...
        period: 'Próximo',
        items: [
          { title: 'Medidas corporales en check-in (cintura, brazos, caderas, piernas)', done: false, priority: 'P1', note: 'Web completo: sección colapsable en CheckInClient.tsx, Zod validation, persiste en WeeklyCheckIn. API mobile acepta los 4 campos en schema. PENDIENTE: UI mobile — el formulario de check-in en mobile no tiene los 4 inputs (waistCm/armsCm/hipsCm/thighsCm). El atleta que usa mobile nunca registra medidas corporales aunque la API las acepta.' },
          ...
          { title: 'Editar perfil de salud desde mobile (peso, talla, FC reposo, lesiones)', done: false, priority: 'P1', note: 'Web tiene /profile con formulario completo editable (PATCH /api/me + /api/me/health). Mobile muestra los datos en Profile tab pero es solo lectura. El atleta mobile no puede actualizar su peso ni FC reposo — los cálculos de TDEE y zonas FC quedan desactualizados. Endpoints web existentes son reutilizables desde mobile con auth JWT.' },
