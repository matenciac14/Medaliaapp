    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'NUT-DASH-01 — ActivityCard en /nutrition: sesion del dia visible en pantalla de nutricion', done: false, priority: 'P2', note: 'La pantalla de nutricion no muestra la sesion de entrenamiento del dia aunque page.tsx ya carga todaySession. Implementar ActivityCard: tipo de sesion (RODAJE_Z2 → "Entrenamiento de fuerza" etc), durationMin, kcal estimadas quemadas (~kcalBurned desde intensity). Mostrar badge "DIA DE ENTRENAMIENTO" cuando hasGymSessionToday o todaySession != null. Data disponible: todaySession.intensity + durationMin (extender select en page.tsx), hasGymSessionToday. Componente nuevo: ActivityCard.tsx. Posicion: antes de MacroCards, primera card del area principal. Archivo: src/app/(athlete)/nutrition/page.tsx + _components/ActivityCard.tsx.' },
          { title: 'NUT-DASH-02 — MacroCards hero siempre visibles en /nutrition (4 targets del dia)', done: false, priority: 'P2', note: 'Los 4 targets del dia (Energia/Proteina/Carbos/Grasas) solo son visibles dentro de NutritionContent (requiere mealPlan activo) o colapsados en TrackingSection. El diseno Figma los muestra como 4 cards prominentes arriba, siempre visibles para cualquier atleta con effectiveNutritionPlan. Implementar: seccion fija "LO QUE TU CUERPO NECESITA HOY" con 4 cards cuando effectiveNutritionPlan != null, independiente de si hay mealPlan. Data disponible en page.tsx: todayKcal, todayProtein, todayCarbs, todayFat. Componente: MacroTargetCards.tsx. Archivo: src/app/(athlete)/nutrition/page.tsx.' },
          { title: 'NUT-DASH-03 — DeficitHero con macro pills: consumido vs restante desglosado por macro', done: false, priority: 'P2', note: 'TrackingSection.tsx muestra kcal totales + barra de progreso, pero no el desglose por macro en formato pill. El diseno Figma separa "Consumido" vs "Te faltan" como hero card, y bajo la barra muestra 3 pills: Proteina X/Y g · Carbos X/Y g · Grasas X/Y g. Implementar: refactor o extension de TrackingSection para incluir la fila de macro pills con progreso individual (azul/naranja/verde) visibles sin expandir. Actualmente los macros solo se ven en la seccion expandida. Archivo: src/app/(athlete)/nutrition/_components/TrackingSection.tsx.' },
          { title: 'NUT-DASH-04 — MealCards compactas: sugerencias del dia con CTA "Ver plan completo"', done: false, priority: 'P2', note: 'El diseno Figma muestra 3 cards horizontales compactas ("QUE PUEDES COMER HOY") con Pre-entreno/Almuerzo/Cena, lista de alimentos y kcal, mas CTA "Ver plan completo (5 comidas) →". El codigo actual tiene NutritionContent con el menu completo en tabs (scroll largo). Implementar: MealSummaryCards.tsx que muestra las primeras 3 comidas del mealPlan del dia actual como cards compactas. CTA "Ver plan completo" expande o navega al NutritionContent. Solo visible cuando hasMealPlan. Condicion: (assignedMealPlan ?? parsedMealPlan) != null. Archivo: src/app/(athlete)/nutrition/_components/MealSummaryCards.tsx + page.tsx.' },
          { title: 'NUT-WATER-01 — WaterLog tracking web: endpoint POST + widget con botones +250ml/+500ml/+1L', done: false, priority: 'P2', note: 'DB-18 WaterLog implementado (userId, date @db.Date, mlLogged Int, @@unique([userId, date])). NutritionPlan.waterMlTarget Int? @default(2000). Pendiente: (1) POST /api/nutrition/water — auth + upsert WaterLog { mlLogged: existente + delta }, (2) GET /api/nutrition/water?date= — retorna mlLogged + waterMlTarget, (3) HydrationWidget.tsx — barra de progreso "X.X L / 2.5 L", 3 botones +250ml/+500ml/+1L (client component con fetch). El diseno Figma tiene el widget completo en la columna derecha del dashboard de nutricion. Paridad mobile pendiente separado. Archivos: src/app/api/nutrition/water/route.ts + src/app/(athlete)/nutrition/_components/HydrationWidget.tsx.' },
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'GAP-02 — sessionMinutes no afecta TDEE: mismo multiplicador para 30 min/sesión que para 90 min/sesión', done: false, priority: 'P3', note: 'Detectado en análisis de flujo B2C 2026-07-14. calculateTDEE() usa solo daysPerWeek para el factor de actividad. sessionMinutes se guarda en HealthProfile como metadata pero no influye en el cálculo energético. Delta real: 4d×30min vs 4d×90min deberían tener multiplicadores distintos. Sugerencia: calcular volumen semanal total (daysPerWeek×sessionMinutes) y ajustar el factor entre bandas (< 120min/sem = sedentario, 120-300 = moderado, etc). No urgente — mejora de precisión TDEE para usuarios con sesiones muy cortas o muy largas.' },
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: 'MOB-NUT-04 — Mobile: apply template a la semana con selector de intensidad por día', done: false, priority: 'P2', note: 'Paridad con ApplyWeekModal de web (NUT-F-01). Modal en mobile: 7 días desde hoy con selector HARD/EASY/REST por día → POST /api/athlete/nutrition/templates/[id]/apply. API ya existe. Depende de MOB-NUT-03. Sin esto el atleta que construye su template en web no puede aplicarlo desde mobile.' },
        ],
      },
    ],
  },
...

  {
    ...
    items: [
      ...
      },
      {
        ...
        priority: 'P1',
        note: 'El handler genérico (POST /api/webhooks/payment) y los use cases de checkout/downgrade ya están. Falta el adaptador concreto: src/infrastructure/billing/wompi.gateway.ts implementando IPaymentGateway — createCoachCheckoutSession(), createAthleteCheckoutSession(), parseWebhookEvent(rawBody, signature). Wompi firma los webhooks con HMAC-SHA256 del event_id. Checkout: POST https://checkout.wompi.co/p/ con redirect_url + amount_in_cents + currency + reference. Una vez implementado: PAYMENT_GATEWAY=wompi en Vercel env + probar con sandbox Wompi. Prerequisito para cobrar en Colombia.',
      },
      {
