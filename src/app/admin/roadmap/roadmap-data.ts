...
export const GROUPS: RoadmapGroup[] = [
...

  {
    ...
    items: [
      ...
      { title: 'DB — Payment.amount Float → Int o Decimal(12,2) para valores monetarios', done: false, priority: 'P2', note: 'Float puede introducir errores de precisión en aritmética monetaria. COP no tiene centavos pero MXN/USD sí. El campo se usa en reduce() y toLocaleString(). Fix: migrar a Int (centavos) o Decimal(12,2) antes de conectar pasarela de pagos Wompi/Stripe.' },
      { title: 'DB — TrainingPlan.goalType String? → GoalType enum para constraint a nivel DB', done: false, priority: 'P3', note: 'GoalType enum existe (RACE_5K|RACE_10K|RACE_HALF_MARATHON|RACE_MARATHON|BODY_RECOMPOSITION|STRENGTH_TRAINING). El campo String? permite valores inválidos llegar a DB. Templates ya usan exactamente estos valores. Fix: cambiar a goalType GoalType? en schema + migración.' },
      { title: 'DB — SessionLog.freeSessionType String? → SessionType enum', done: false, priority: 'P3', note: 'freeSessionType se guarda como string libre y se filtra con where: { freeSessionType: "FUERZA" } en gym/history/route.ts. Un typo no daría error en DB ni en Prisma. Fix: cambiar a SessionType? enum — ya está definido en schema.' },
      ...
      { title: 'PERF-01 — GET /api/nutrition/foods carga toda la librería sin límite ni búsqueda', done: false, priority: 'P2', note: 'prisma.food.findMany({ where: { isActive: true } }) sin take ni cursor. Hoy ~50 alimentos — inofensivo. A 300-500 alimentos (meta realista) cada apertura de nutrición descarga todo. Fix: agregar ?search= con where: { name: { contains: q, mode: "insensitive" } } + take: 50. Mismo fix para /api/mobile/nutrition/foods/route.ts.' },
      { title: 'PERF-02 — cron/session-reminder carga TODOS los weeks de TODOS los planes activos en memoria', done: false, priority: 'P2', note: 'prisma.trainingPlan.findMany({ weeks: { sessions: {} } }) sin take — a 100 usuarios × 18 semanas × 7 sesiones = 12,600 filas. El filtro weekNumber === currentWeek se hace en JS post-query. Fix: calcular la fecha de lunes de la semana actual en UTC, luego filtrar PlannedSession directamente con date = monday + where dayOfWeek = 1, joinear con plan/user — elimina la carga de semanas innecesarias.' },
    ],
  },
    ...
    phases: [
      ...
      // ── GYM & TRACKING ────────────────────────────────────────────────────────
      {
        ...
        items: [
          ...
          { title: 'BUG-025 — Gym: POST /api/gym/session/complete rechaza CUID/slugs con "Invalid UUID"', done: false, priority: 'P0', note: 'Detectado en test E2E 2026-07-01. El schema Zod usa z.string().uuid() pero Prisma genera CUIDs y los templates públicos usan slugs. Fix: cambiar .uuid() por .min(1) en 5 campos de gym/session/complete/route.ts (líneas 9, 17, 18, 24, 25). También hay .uuid() en api/log/session/route.ts:7 (plannedSessionId) y api/mobile/log/session/route.ts:9 (sessionId) — mismo fix. Bloquea la feature gym completamente.' },
          { title: 'BUG-026 — Dashboard: dots de consistencia semanal muestran 0 tras completar sesión de gym (modo FREE)', done: false, priority: 'P2', note: 'Detectado en test E2E 2026-06-30 (PARCIAL). Reproducción: usuario FREE sin plan activo, completa sesión gym, vuelve al dashboard → contador "1/3 días" correcto pero dots visuales muestran 0. Root cause probable: timezone mismatch entre GymSession.date (setHours(0,0,0,0) en servidor UTC) y weekStart/weekEnd calculados en dashboard. También verificar si weekGymSessions query incluye la sesión del día actual.' },
      ...
      // ── COACH ─────────────────────────────────────────────────────────────────
      {
        ...
        items: [
          ...
          { title: 'SEC-01 — POST /api/messages y /api/mobile/messages sin validar relación coach-atleta', done: false, priority: 'P1', note: 'Cualquier usuario autenticado puede enviar mensajes a cualquier otro userId de la plataforma sin verificar que exista una relación CoachAthlete. Fix: antes de crear el Message, verificar prisma.coachAthlete.findFirst({ where: { OR: [{ coachId: fromId, athleteId: toId }, { coachId: toId, athleteId: fromId }] } }) → 403 si no existe. Aplica a ambos endpoints: api/messages/route.ts (POST) y api/mobile/messages/route.ts (POST).' },
        ],
      },
