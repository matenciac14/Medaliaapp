...
export const GROUPS: RoadmapGroup[] = [
...

  {
    ...
    items: [
      ...
      { title: 'DB — Índices faltantes: SessionLog.completedAt, CoachProfile.isPublic, WorkoutTemplate(isPublic, isActive)', done: false, priority: 'P2', note: 'SessionLog.completedAt: usado en admin/metrics (gte: sevenDaysAgo) y dashboard (orderBy). Solo existe @@index([userId]). CoachProfile.isPublic: usado en /coaches listing y /p/[slug]. WorkoutTemplate(isPublic,isActive): composite query en gym/assign y mobile/gym/templates. Fix: agregar en nueva migración SQL — CREATE INDEX CONCURRENTLY.' },
      ...
      { title: 'DB — CoachAthlete sin updatedAt — sin auditoría de cuándo cambió el status', done: false, priority: 'P3', note: 'status (ACTIVE|PENDING|INACTIVE) se consulta frecuentemente pero no hay forma de saber cuándo se activó o desactivó un atleta. Fix: agregar updatedAt DateTime @updatedAt a CoachAthlete + migración.' },
    ],
  },
    ...
    phases: [
      ...
      // ── DATOS & CÁLCULOS ──────────────────────────────────────────────────────
      {
        ...
        items: [
          ...
          { title: 'BUG-022 — HealthProfile.age almacenado, no calculado — se vuelve stale después del onboarding', done: false, priority: 'P1', note: 'age se guarda en onboarding y nunca se actualiza automáticamente. Se usa directamente en calculateTDEE en process-check-in.use-case.ts:242, generate-plan.use-case.ts:76, generate-meal-plan.ts:110. Cada cumpleaños el TDEE y las zonas HR quedan incorrectos sin que el usuario lo note. Fix: eliminar columna age de HealthProfile, calcular en runtime con calcAge(dob) donde se necesite. dob ya existe en el modelo.' },
          { title: 'BUG-023 — InviteCode.usedBy es String? sin FK — referencia huérfana si el usuario se elimina', done: false, priority: 'P2', note: 'usedBy guarda el userId como string libre, sin relación en Prisma. Si el atleta es eliminado, el código queda con un ID apuntando a nada. Admin page en /admin/invite-codes hace lookup manual con where: { id: { in: usedByIds } } — frágil. Fix: convertir usedBy a relación opcional @relation(fields:[usedBy], references:[id]) con onDelete: SetNull.' },
        ],
      },
