...
export const GROUPS: RoadmapGroup[] = [
...

  {
    ...
    items: [
      ...
      { title: 'Admin: métricas de negocio reales en /admin/metrics (usuarios activos 7d, tasa onboarding, adherencia plataforma, coaches con atletas)', done: false, note: 'Hoy solo muestra 2 contadores. Prioridad: datos que sirven para tomar decisiones de producto.' },
      { title: 'Admin: búsqueda y filtros en /admin/users (nombre, email, rol, fecha registro)', done: false, note: 'Con 50+ usuarios la tabla plana es inmanejable. Cursor pagination + server-side search.' },
      { title: 'Admin: perfil individual /admin/users/[id] (HealthProfile, plan activo, último check-in, historial de rol)', done: false, note: 'Permite diagnosticar problemas de un usuario sin ir a la DB directamente.' },
      { title: 'Admin: sincronizar features al cambiar rol (ChangeRoleButton → roles como presets de features)', done: false, note: 'Hoy solo actualiza la columna role. Fix bloqueante para operaciones confiables desde admin.' },
      { title: 'Admin: MRR, churn y funnel trial→Pro en dashboard financiero', done: false, note: 'Suma suscripciones activas + fees coaches. Gráfica histórica. Necesario para medir revenue real.' },
    ],
  },
...

  {
    ...
    items: [
      ...
      { title: 'Wompi (Colombia): suscripción atleta Pro $15/mes', done: false, note: 'Wompi primero por mercado principal. Checkout → webhook → activa PRO en DB.' },
...

  {
    ...
    phases: [
      ...
      // ── ADMIN ─────────────────────────────────────────────────────────────────
      {
        ...
        period: 'Urgente',
        items: [
          { title: 'ADMIN-BUG-01 — /admin/settings hardcodeado con datos desactualizados', done: false, priority: 'P2', note: 'Muestra "Next.js 15" (es 16), "Vercel + Neon (pendiente)" (ya deployado), "Google OAuth pendiente" (implementado), "AWS SES" (se usa Resend). Fix: actualizar valores o eliminar la sección y mostrar solo estado real.' },
          { title: 'ADMIN-BUG-02 — /admin/metrics solo muestra 2 contadores — sin contexto real', done: false, priority: 'P2', note: 'Faltan: usuarios activos (logueados últimos 7d), adherencia promedio de la plataforma, coaches con atletas activos, tasa de onboarding completado. Fix: expandir con queries útiles para el negocio.' },
          { title: 'ADMIN-BUG-03 — Sin búsqueda ni filtros en /admin/users y /admin/coaches', done: false, priority: 'P2', note: 'Con 50+ usuarios la tabla es inmanejable. Fix: input de búsqueda por nombre/email + filtro por rol en users, búsqueda por nombre en coaches.' },
          { title: 'ADMIN-BUG-04 — Sin acceso a perfil individual de atleta desde admin', done: false, priority: 'P3', note: 'Solo se ve la tabla, no hay link al perfil completo. Fix: link en columna nombre → /admin/users/[id] con vista de HealthProfile, plan activo, último check-in.' },
          { title: 'ADMIN-BUG-05 — ChangeRoleButton no resetea feature flags al cambiar rol', done: false, priority: 'P1', note: '/api/admin/user/[id]/role solo actualiza la columna role — no toca features. Un atleta cambiado a COACH mantiene featurePlan=true y featureCheckin=true. Fix: sincronizar features según el rol destino (COACH → solo featureCoach=true, resto false).' },
          { title: 'ADMIN-BUG-06 — Plan FREE y PRO activan features idénticas en /api/admin/users/[id]/plan', done: false, priority: 'P1', note: 'Rama FREE: enableFeatures([plan, checkin, nutrition, progress, log, gym]) — igual que PRO. Fix: FREE debe activar solo featureLog + featurePlan básico, sin nutrition ni gym (ver modelo de negocio).' },
          { title: 'ADMIN-BUG-07 — /admin/help expone credenciales de producción en texto plano', done: false, priority: 'P1', note: 'admin@medaliq.com/admin123!, coach@medaliq.com/coach123, miguel@medaliq.com/atleta123 visibles en la página sin auth adicional. Fix: eliminar credenciales del HTML — nunca en texto plano, ni en herramientas internas.' },
          { title: 'ADMIN-BUG-08 — Sidebar admin 9 items — ilegible en laptops y sin agrupar', done: false, priority: 'P2', note: 'AdminSidebarClient.tsx lista 9 links planos. Fix: agrupar en secciones (Usuarios / Plataforma / Config) o colapsar items secundarios.' },
          { title: 'ADMIN-BUG-09 — /admin/plans y /admin/metrics no aparecen en el sidebar', done: false, priority: 'P2', note: 'Las páginas existen y funcionan pero no tienen link de navegación. Fix: agregar al sidebar o consolidar en /admin/metrics.' },
          { title: 'ADMIN-BUG-10 — Sin paginación en tablas de usuarios, coaches y suscripciones', done: false, priority: 'P3', note: 'Con 100+ usuarios la tabla carga todo en memoria. Fix: cursor-based pagination con page/limit en los endpoints.' },
          { title: 'ADMIN-BUG-11 — Sin confirmación antes de cambiar rol o desactivar usuario', done: false, priority: 'P3', note: 'ChangeRoleButton ejecuta el PATCH directamente sin confirm dialog. Fix: modal de confirmación con resumen del cambio.' },
        ],
      },
