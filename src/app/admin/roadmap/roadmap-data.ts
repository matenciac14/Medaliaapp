...
export const GROUPS: RoadmapGroup[] = [
    ...
    phases: [
      {
        ...
        items: [
          ...
          { title: 'Open Graph tags (og:title, og:description, og:image) para preview en WhatsApp/LinkedIn/Twitter', done: false, priority: 'P0', note: 'layout.tsx: openGraph + twitter card + alternates/hreflang configurados. og:image apunta a /og-image.png pero el archivo NO existe en /public/ — el preview en WhatsApp/LinkedIn mostrará imagen en blanco. Pendiente: crear /public/og-image.png (1200×630px).' },
          ...
          { title: 'Eliminar toda referencia a "Trial" o "30 días gratis" del copy de la landing — producto sin trial', done: false, priority: 'P0', note: 'El modelo de negocio no tiene trial — tiene Free permanente. Promesa falsa activa en producción. Cambios exactos: CTAs de coach → "Empieza gratis →". Sección garantía → "Empieza gratis. Crece cuando tengas más atletas." Paso 1 del flujo → "Sin tarjeta, sin compromiso. Empieza gratis con hasta 5 asesorados." Paso 5 (título) → "Elige tu plan cuando estés listo". Bloque tier Trial en pricing atleta → eliminar completo, dejar solo Free y Pro. Identificados 8+ puntos en código. Bloquea conversión: coach se registra esperando 30 días gratis completos → encuentra Free limitado → percepción de engaño.' },
          { title: 'Corregir paso 2 del flujo "Cómo funciona" — el sistema NO genera planes automáticamente', done: false, priority: 'P0', note: 'Antes: "Tu atleta entra, completa su perfil y Medaliq genera su plan automáticamente." Después: "Tu atleta entra, completa su perfil y queda listo en minutos. Tú le asignas el plan desde tu panel." Error de expectativa crítico: atleta onboarding sin plan → experiencia de app rota → churn inmediato. Aplica tanto al código (landing) como al diseño Figma.' },
        ],
      },
      {
        id: 'landing-p1',
        ...
        items: [
          ...
          { title: 'Corregir copy mobile en pricing atleta PRO', done: false, priority: 'P1', note: 'Antes: "App mobile (próximamente)". Después: "Webapp móvil incluida · App nativa en desarrollo". La webapp responsive ya existe y funciona desde cualquier celular. Las apps nativas (Play Store/App Store) están en desarrollo. El framing actual hace que los coaches duden de que sus atletas puedan usar el producto desde celular — la pregunta #1 en el proceso de venta.' },
          { title: 'Google OAuth en /login visible pero en standby — activar o quitar de la UI', done: false, priority: 'P1', note: 'El botón "Continuar con Google" está visible en la pantalla de login pero el flujo está en standby (falta GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET en Google Cloud Console + Vercel). Un coach que intente usarlo en este momento se encontrará con un error. Decisión: activar el OAuth o ocultar el botón hasta que esté listo.' },
          ...
          { title: 'Actualizar features del tier Free del atleta en pricing — alinear con modelo real', done: false, priority: 'P1', note: 'Antes: "Dashboard básico / Log de entrenamientos manual / Sin plan adaptativo". Después: "Dashboard + log de entrenamientos / Registro de nutrición y gym / Sin plan adaptativo ni métricas de progreso". El Free actual incluye nutrición y gym — la landing subestima el producto. Un coach que evalúa la plataforma cree que sus atletas recibirán menos de lo real.' },
...

  {
    ...
    borderColor: '#fed7aa',
    items: [
      { title: 'Limpiar usuarios de prueba de producción antes del lanzamiento', done: false, priority: 'P0', note: 'Scripts listos: prisma/seed.prod.ts (admin + ejercicios + templates, sin test users) y scripts/cleanup-test-users.sql (elimina 17 usuarios de prueba). PENDIENTE: ejecutar cleanup-test-users.sql en Neon prod antes del lanzamiento público. Marcar done: true solo cuando la ejecución se confirme.' },
...

  {
    ...
    items: [
      ...
      { title: 'TIER-MODEL — Actualizar computeAthleteFeatures() al modelo de tiers definitivo', done: false, priority: 'P0', note: 'Nuevo modelo FREE definido en análisis de mercado (2026-07-03): FREE = capa de registro { plan:true, checkin:false, nutrition:true, progress:false, log:true, gym:true }. Gate de pago: checkin (ajustes automáticos + notificaciones) y progress (métricas históricas). Hoy FREE tiene plan:false, nutrition:false, gym:false — incorrecto para el modelo de negocio. Ejecutar ANTES de activar Wompi. Archivo: src/domain/subscription/tier-features.ts → computeAthleteFeatures() FREE case. Incluir tests actualizados en tier-features.test.ts.' },
      { title: 'BILLING-PREP — Activar getUserPlan() real + migración usuarios beta', done: false, priority: 'P0', note: 'Hoy getUserPlan() hardcodea PRO para todos (intencional en beta). Al activar Wompi: (1) getUserPlan() debe leer de UserSubscription.tier en DB en lugar de hardcodear PRO. (2) Script de migración: todos los usuarios registrados antes del launch date → SubscriptionTier.TRIAL por 30 días. (3) Nuevos usuarios post-launch → flujo normal TRIAL → FREE/PRO. Ejecutar ANTES de activar el webhook de Wompi. Archivo: src/lib/config/user-config.ts → getUserPlan().' },
    ...
    phases: [
      ...
      // ── UI / UX ATLETA ────────────────────────────────────────────────────────
      {
        ...
        items: [
          ...
          { title: 'BUG-046 — Check-in viernes: mensaje "espera al viernes" cuando HOY es viernes', done: false, priority: 'P2', note: 'QA-2026-07-03: /checkin muestra "Se activa el viernes · Tu próximo check-in es el viernes, 3 de julio" siendo hoy viernes 3 de julio. El usuario ve "espera" pero ya ES el día. Posible causa: comparación de fecha en UTC vs timezone local (America/Bogota = UTC-5). Si son las 9am del viernes en Colombia, en UTC son las 14:00 del viernes — debería activar. Verificar lógica de isCheckInAvailable() en /app/checkin/page.tsx. Mitigado por botón "Abrir check-in de todas formas".' },
          ...
          { title: 'BUG-048 — Onboarding paso 2: sin feedback de validación al usuario (valores inválidos bloqueados en silencio)', done: false, priority: 'P2', note: 'QA-2026-07-03: Con edad=-5, talla=0, peso=999 el botón "Siguiente →" no avanza y no muestra ningún mensaje de error. Los inputs type="number" no tienen atributos min/max. La validación existe server-side o en el submit handler pero no da feedback visual al usuario. Fix: agregar inline error messages bajo cada campo con valores fuera de rango. UX crítica: el usuario no sabe por qué no puede continuar.' },
          { title: 'BUG-049 — Sidebar idioma persiste en PT (Portugués) entre sesiones de usuarios en ES', done: false, priority: 'P2', note: 'QA-2026-07-03: El sidebar de navegación muestra labels en Portugués ("Sair", "Início", "Meu Plano", "Nutrição") para múltiples usuarios (Ana, Miguel, Juan, Sebastián) aunque el contenido de la página está en español. El selector de idioma muestra "PT" activo. La cookie de idioma queda persistida en PT desde sesiones anteriores y no se limpia al hacer logout o al login de un nuevo usuario. Fix: al hacer login, verificar si hay cookie de idioma y solo resetear a DEFAULT_LOCALE="es" si el usuario no tiene preferencia explícita guardada en DB.' },
          { title: 'BUG-050 — /checkin UI muestra "Semana 6 · 22 may" aunque el check-in se guardó correctamente en semana 12', done: false, priority: 'P1', note: 'CÓDIGO LOCALIZADO 2026-07-03. Archivo: src/app/(athlete)/checkin/page.tsx línea 87-88. El weekNumber en DB se guarda CORRECTO (weekNumber=12) por process-check-in.use-case.ts:81 que llama getPlanWeekNumber(startDate, totalWeeks). El bug es SOLO de presentación: línea 87 busca `activePlan.weeks.find(w => w.weekNumber === 12)` pero el seed solo tiene PlanWeek para semanas 1-6 → find() retorna undefined → línea 88 fallback: `activePlan.weeks[activePlan.weeks.length - 1]` = semana 6. Línea 109 construye el label con weekData de la semana 6 → "Semana 6 · 22 de may". FIX: en línea 88, si find() retorna undefined, construir un weekData sintético con currentWeek y fechas calculadas desde plan.startDate + (currentWeek-1)*7días, en lugar de caer a la última semana existente.' },
          { title: 'BUG-051 — Botón "Actualizar datos de esta semana" en /checkin redirige al home en lugar de abrir formulario', done: false, priority: 'P2', note: 'QA-2026-07-03: Después de enviar el check-in, la pantalla de resumen muestra botón "Actualizar datos de esta semana". Al hacer click, navega a /dashboard (home) en lugar de mostrar el formulario editable del check-in. Investigar CheckInClient.tsx — el handler del botón posiblemente tiene router.push("/") o un link padre captura el evento antes del handler. El usuario no puede corregir datos una vez enviados.' },
          { title: 'BUG-052 — Click en Guía de alimentos de /nutrition navega a /progress (event bubbling)', done: false, priority: 'P2', note: 'QA-2026-07-03: En /nutrition, al hacer click en un ítem de la sección "Guía de alimentos" (después de haber expandido el acordeón "LO QUE COMÍ HOY"), la página navega a /progress. El click se propaga a un elemento padre que tiene un link o handler de navegación. Posible causa: el acordeón y la guía comparten un contenedor con onClick que navega a /progress cuando está en estado expandido. Verificar NutritionClient.tsx — estructura de event handlers en el acordeón vs la guía.' },
          { title: 'BUG-053 — /nutrition: sin botón eliminar alimento registrado en el log del día (web)', done: false, priority: 'P2', note: 'QA-2026-07-03: El acordeón "LO QUE COMÍ HOY" muestra el conteo de registros y las kcal consumidas, pero al expandir no expone botón de eliminar para cada ítem individual. El usuario no puede corregir registros de comida desde la web. Verificar si la funcionalidad existe en mobile — si solo está en mobile es paridad pendiente. NutritionClient.tsx: buscar el renderizado del log expandido y agregar botón de eliminar con DELETE /api/nutrition/log/[id].' },
          { title: 'BUG-054 — /gym/session: click en "×" de set de ejercicio colapsado navega al home', done: false, priority: 'P1', note: 'QA-2026-07-03: En la sesión activa de gym (/gym/session), al intentar registrar un set "×" en un ejercicio que no está expandido activamente, el evento navega a https://www.medaliq.com/ (home) en lugar de registrar la serie. Solo funciona registrar sets en el ejercicio que está actualmente expandido. Causa probable: el elemento "×" dentro del acordeón colapsado tiene un link o un ancestor con href="/" que captura el click antes del handler de registrar serie. Verificar GymSessionClient.tsx — los set markers dentro de acordeones colapsados tienen event bubbling hacia un elemento clickeable que apunta al home. Fix: stopPropagation() en el handler de click del set marker, o reestructurar el DOM para que el link padre no envuelva los set markers.' },
          { title: 'BUG-055 — Peso inconsistente: /plan y /profile (75kg) vs /dashboard y /progress (73.5kg)', done: false, priority: 'P2', note: 'CÓDIGO LOCALIZADO 2026-07-03. CAUSA RAÍZ: syncWeight() en process-check-in.use-case.ts:233-234 tiene guardia `if (Math.abs(newWeight - prev) < 0.5) return`. La variable `prev` = peso del check-in anterior (seed wn:6 = 73.8kg). El atleta ingresó 73.5kg → delta = 0.3 < 0.5 → syncWeight hace early return SIN actualizar HealthProfile.weightKg. Resultado: HealthProfile.weightKg queda en 75kg (valor del seed). /plan/page.tsx:129 y /profile usan `profileData.weightKg` (HealthProfile = 75kg). /dashboard/page.tsx:348 usa `lastCheckIn?.weightKg ?? profile?.weightKg` (WeeklyCheckIn = 73.5kg). FIX: en syncWeight() línea 233, comparar newWeight contra `profile.weightKg` (HealthProfile base) en lugar de contra el check-in previo. Con |73.5 - 75| = 1.5 > 0.5 el sync ejecutaría y HealthProfile quedaría actualizado.' },
          ...
          { title: 'BUG-057 — Dashboard "0/0 completadas" ignora SessionLog: atleta que entrena libremente no ve progreso semanal', done: false, priority: 'P2', note: 'CÓDIGO LOCALIZADO 2026-07-03. Archivo: src/app/(athlete)/dashboard/page.tsx líneas 367-369. `completedCount = selectedPlanWeekSessions.filter(s => s.log && s.type !== DESCANSO).length` y `totalTraining = selectedPlanWeekSessions.filter(s => s.type !== DESCANSO).length`. Ambos solo cuentan PlannedSession — SessionLog (actividades libres via /log/run) no entra. FIX: consultar también SessionLog de la semana actual y sumar al contador, o mostrar un contador adicional "N actividades libres esta semana" en DailySessionCard.' },
          { title: 'BUG-058 — /plan semanas sin PlanWeek en DB muestran "Día de descanso" sin distinguir de plan incompleto', done: false, priority: 'P1', note: 'CÓDIGO LOCALIZADO 2026-07-03. Dos archivos. (1) src/app/(athlete)/plan/page.tsx:155-179: el mapeo `activePlanData.weeks.map(...)` solo incluye PlanWeek que existen en DB → semanas 7-18 no aparecen. (2) src/app/(athlete)/plan/_components/PlanClient.tsx:1207: `weeks.find(w => w.weekNumber === selectedWeekNum) ?? null` → null para semana 12. PlanClient.tsx:1322-1339: el else de selectedSession renderiza "😴 Día de descanso · Aprovecha para recuperar bien hoy" igual para (a) día de tipo DESCANSO y (b) semana sin PlanWeek. No hay distinción entre ambos casos. FIX en PlanClient.tsx: agregar condición `if (!week)` antes del else de selectedSession → mostrar banner "Tu coach aún no ha definido las sesiones de esta semana" en lugar del emoji de descanso.' },
          { title: 'BUG-059 — /plan adherencia histórica 0% en semanas con plan vacío: métrica engañosa para atleta', done: false, priority: 'P2', note: 'QA-2026-07-03: El historial de adherencia en /plan muestra 0% para semanas 10-14. Técnicamente correcto (0 sesiones planificadas / 0 sesiones completadas = no aplica), pero se muestra como 0% que parece "mal desempeño". Las semanas donde el plan no tiene sesiones deberían mostrarse como "—" o "sin datos" en lugar de 0%. Fix: en el cálculo de adherencia, si denominador === 0 → mostrar null/"—" en lugar de 0%. Verificar el componente de adherencia en /plan y en /progress.' },
          ...
          { title: 'BUG-063 — /gym B2B: atleta sin rutina asignada bloqueado sin acceso a plantillas (early return en gym/page.tsx:95-106)', done: false, priority: 'P2', note: 'CÓDIGO LOCALIZADO 2026-07-03. Archivo: src/app/(athlete)/gym/page.tsx líneas 89-107. Flujo: if (!assigned) → busca coachRelation → if (coachRelation) return <"Tu coach gestionará tu rutina"> (línea 96, EARLY RETURN). B2C sin coach no entra al if(coachRelation) → cae a línea 109 y ve plantillas públicas. B2B con coach pero sin AssignedWorkout → entra al if(coachRelation) y queda bloqueado. El condicional no distingue entre "B2B sin rutina aún" y "B2B con rutina asignada". FIX: eliminar el early return en línea 96-106. Mostrar plantillas públicas en ambos casos (B2C y B2B sin rutina), con banner diferente: si coachRelation → "Tu coach aún no te asignó una rutina — puedes usar estas plantillas mientras tanto".' },
          ...
          { title: 'BUG-065 — /log/run: sin redirect ni confirmación visual tras guardar corrida', done: false, priority: 'P2', note: 'QA-2026-07-03: Después de hacer click en "Guardar corrida ✓", el formulario se vacía pero no hay redirect automático ni toast de confirmación clara. El usuario no sabe con certeza si se guardó correctamente. Fix: después del POST exitoso a /api/sessions/log, hacer router.push("/log/history") o mostrar toast "Corrida guardada ✓" y resetear el formulario. Verificar LogRunClient.tsx — el onSuccess del submit.' },
          ...
          { title: 'BUG-067 — /profile: fecha de nacimiento vacía pero edad muestra "30 años" (edad hardcodeada en HealthProfile.age)', done: false, priority: 'P2', note: 'QA-2026-07-03: /profile muestra birthDate = "—" (null) pero edad = "30 años". La edad se almacena en HealthProfile.age (campo numérico) en lugar de calcularse desde birthDate. Si el usuario actualiza su fecha de nacimiento en el futuro, la edad no se actualizaría automáticamente. Fix: (a) calcular edad dinámicamente desde birthDate cuando esté disponible, o (b) mostrar campo de fecha de nacimiento editable en el formulario de datos de salud y sincronizar age al guardar. Verificar ProfileClient.tsx y el formulario de edición — birthDate no está en los campos editables.' },
          ...
          { title: 'BUG-070 — /new-goal: pre-selección de meta no corresponde al deporte elegido en onboarding', done: false, priority: 'P2', note: 'QA-2026-07-03: Usuario elige "Running" en onboarding pero /new-goal pre-selecciona "Fitness general". La pre-selección no usa el activityType guardado en el onboarding. Archivo: src/app/(athlete)/new-goal/_components/NewGoalClient.tsx — defaultGoal no mapea RUNNING→5K/10K. Fix: cuando activityType=RUNNING, pre-seleccionar RACE_5K o RACE_10K según el sportGoal guardado.' },
          { title: 'BUG-071 — /new-goal: "Error generando plan" (HTTP 500) — WorkoutDay FK no existe en producción', done: false, priority: 'P0', note: 'QA-2026-07-03: POST /api/plan/new devuelve 500. Causa raíz: generate-plan.use-case.ts:44-50 define FUERZA_CORREDOR_DAY con IDs hardcodeados ("system-fuerza-corredor-base", "system-fuerza-corredor-especifico"). Estos IDs son FK a WorkoutDay creados solo en seed de dev — NO existen en producción. La transacción createSessions falla por violación FK constraint. Fix: (a) ejecutar seed de WorkoutDay del sistema en producción como migración de datos, o (b) hacer workoutDayId condicional: verificar existencia antes de asignar. Archivo: src/domain/plan/generate-plan.use-case.ts:175 + src/infrastructure/db/plan.repository.ts:149-162.' },
        ],
      },
      ...
      // ── ONBOARDING UX — mejoras identificadas QA 2026-07-03 ──────────────────
      {
        ...
        period: 'Próximo sprint',
        items: [
          { title: 'UX-01 — Simplificar paso 1 de onboarding: de 4 opciones a 2 (Ejercicios / Running)', done: false, priority: 'P1', note: 'QA-2026-07-03: Las 4 opciones actuales (Ejercicios, Running, Ejercicios+Running, Solo trackear) generan fricción. El usuario promedio LatAm se identifica con "voy al gym" o "corro". Propuesta: 2 opciones — Ejercicios (sub-pregunta interna: objetivo de composición corporal) y Running (sub-pregunta: distancia objetivo + fecha). "Ejercicios+Running" y "Solo trackear" son edge cases que se manejan post-onboarding. Archivos: src/app/onboarding/page.tsx, src/app/onboarding/_types.ts.' },
          { title: 'UX-02 — Cambiar label "Talla (cm)" por "Altura (cm)" en datos personales del onboarding', done: false, priority: 'P3', note: 'QA-2026-07-03: "Talla" es terminología médica/formal. "Altura" es el término natural en LatAm. Cambio de 1 línea en el label. Archivo: src/app/onboarding/page.tsx — StepPhysical component, label del input de cm.' },
          { title: 'UX-03 — /new-goal: calcular semanas automáticamente desde fecha evento, no mostrarlas como opción fija', done: false, priority: 'P1', note: 'QA-2026-07-03: Hoy se muestran "8 semanas", "12 semanas" como etiqueta de cada meta. El usuario no debería elegir semanas — el sistema las calcula: si el atleta tiene fecha de evento, semanas = (fechaEvento - hoy) / 7. Si no tiene fecha, usar default por distancia (5K=8, 10K=12). Restricción mínima: al menos 4 semanas desde hoy. Restricción máxima: no fechas pasadas. Archivo: src/app/(athlete)/new-goal/_components/NewGoalClient.tsx.' },
          { title: 'UX-04 — /new-goal: CTA "Generar plan" → "Ir al dashboard" (no generar plan en este punto)', done: false, priority: 'P1', note: 'QA-2026-07-03: El CTA actual llama a /api/plan/new que falla (BUG-071). Pero incluso si funcionara, el plan lo debe asignar el coach o se crea en un flujo posterior — /new-goal está deprecado (ARCH-01). El CTA debe guardar solo la meta elegida (goalType + raceDate en HealthProfile) y redirigir al dashboard igual que el onboarding de gym. Archivos: src/app/(athlete)/new-goal/_components/NewGoalClient.tsx, src/app/api/plan/new/route.ts.' },
        ],
      },
      ...
      // ── PERSISTENCIA & TRANSACCIONES — Auditoría 2026-07-03 ──────────────────
      {
        ...
        items: [
          { title: 'PERSIST-01 — FoodLog @@unique([userId,foodId,date,mealType]) bloquea registrar el mismo alimento 2× en la misma comida', done: false, priority: 'P0', note: 'Caso de uso real: comer arroz al almuerzo, luego agregar más arroz. El segundo POST devuelve P2002 silencioso → el registro se pierde sin feedback al usuario. Archivos: api/nutrition/log/route.ts + api/mobile/nutrition/log/route.ts. Fix: usar upsert sumando gramos, o quitar foodId del constraint y manejar varias filas por (userId, date, mealType), o al menos capturar P2002 y devolver mensaje descriptivo.' },
          { title: 'PERSIST-02 — auth/register: user.create fuera de $transaction — usuario huérfano si la tx de userSubscription/coachProfile falla', done: false, priority: 'P0', note: 'Archivo: api/auth/register/route.ts líneas 43-84. user.create (línea 43) se ejecuta antes de la transacción que crea userSubscription + coachProfile. Si la tx falla, queda un User en DB sin suscripción ni perfil de coach. Fix: mover user.create dentro del $transaction.' },
          { title: 'PERSIST-03 — nutrition/generate-meals: 3 writes independientes sin $transaction (foodProfile + nutritionPlan + mealPlan)', done: false, priority: 'P1', note: 'Archivos: api/nutrition/generate-meals/route.ts + api/mobile/nutrition/generate-meals/route.ts. Promise.all([foodProfile.upsert, nutritionPlan.upsert]) + mealPlan.upsert separado. Si falla mealPlan, el atleta queda con nutritionPlan actualizado pero sin mealPlan generado. Fix: envolver los 3 writes en $transaction.' },
          { title: 'PERSIST-04 — nutrition/generate: nutritionPlan.upsert + enableFeatures sin $transaction — features desincronizadas si falla', done: false, priority: 'P1', note: 'Archivo: api/nutrition/generate/route.ts líneas 35-61. Si enableFeatures falla, el atleta tiene NutritionPlan pero sin featureNutrition/featureProgress activas. Fix: envolver en $transaction o incluir enableFeatures dentro del bloque atómico.' },
          { title: 'PERSIST-05 — gym/assign + coach/gym/assign: updateMany(isActive:false) + create sin $transaction → rutinas activas duplicadas', done: false, priority: 'P1', note: 'Archivos: api/gym/assign/route.ts líneas 30-44 + api/coach/gym/routines/[id]/assign/route.ts líneas 56-81. Race condition: dos requests concurrentes pasan el updateMany, ambos desactivan rutinas y ambos crean una nueva → atleta con 2 rutinas activas. Fix: envolver en $transaction.' },
          { title: 'PERSIST-06 — log/session: race condition en check de idempotencia (findUnique + create) → P2002 propagado como 500', done: false, priority: 'P1', note: 'Archivos: api/log/session/route.ts líneas 56-78 + api/mobile/log/session/route.ts líneas 73-93. Si dos requests concurrentes pasan el findUnique con existing=null, ambos intentan crear con el mismo plannedSessionId (@unique) → el segundo falla con P2002 y el usuario ve 500. Fix: usar upsert o capturar P2002 específicamente y devolver el log existente.' },
          { title: 'PERSIST-07 — gym/session/complete: sin check de idempotencia → GymSession @@unique puede fallar en doble submit', done: false, priority: 'P1', note: 'Archivo: api/gym/session/complete/route.ts líneas 207-310. No hay verificación previa al gymSession.create. Si el atleta reintenta (timeout de red), viola @@unique([athleteId, date, assignedWorkoutId]) o @@unique([athleteId, date, plannedSessionId]) → error 500 sin mensaje descriptivo. Fix: agregar findFirst previo o catch P2002 específico.' },
          { title: 'PERSIST-08 — coach/clients/link: check+create sin $transaction → dos coaches pueden vincular al mismo atleta simultáneamente', done: false, priority: 'P1', note: 'Archivo: api/coach/clients/link/route.ts líneas 29-48. Check de relación existente, check de otro coach, y create no están en transacción. Fix: envolver en $transaction o confiar en unique constraint con manejo explícito de P2002.' },
          { title: 'PERSIST-09 — GET /api/mobile/nutrition hace write (lazy-init de NutritionPlan) → viola REST, race condition con unique constraint', done: false, priority: 'P1', note: 'Archivo: api/mobile/nutrition/route.ts líneas 68-93. El handler GET ejecuta nutritionPlan.upsert si no existe. Dos GET simultáneos pueden violar unique constraint. Fix: mover lazy-init a endpoint POST separado o a un use case invocado solo en onboarding.' },
          { title: 'PERSIST-10 — metrics/log POST: upsert sobreescribe con null campos no enviados — pérdida de métricas diarias', done: false, priority: 'P1', note: 'Archivo: api/metrics/log/route.ts líneas 29-47. El update incluye `weightKg: weightKg ?? null` para todos los campos. Si el atleta envía solo peso hoy, los campos sleepHours/energyLevel guardados anteriormente se sobreescriben con null. Fix: omitir del update object los campos que vengan undefined en el body.' },
          { title: 'PERSIST-11 — painDescription no se guarda desde web: schema de validación del check-in web omite el campo', done: false, priority: 'P1', note: 'Archivo: api/checkin/route.ts — el schema Zod no incluye painDescription. El repositorio (check-in.repository.ts:43) sí lo soporta. El atleta web no puede describir su dolor, solo reportar el nivel. Fix: agregar painDescription z.string().optional() al schema de checkin web.' },
          ...
          { title: 'PERSIST-14 — mobile/profile PATCH: healthProfile.update() falla con P2025 si HealthProfile no existe', done: false, priority: 'P2', note: 'Archivo: api/mobile/profile/route.ts línea 79. Usa update() directo. Si el usuario completó auth pero no onboarding (HealthProfile no creado), el PATCH lanza 500. Fix: usar upsert o verificar existencia con findUnique previo.' },
      ...
      // ── PERFORMANCE & FAT ROUTES — Auditoría 2026-07-03 ─────────────────────
      {
        ...
        items: [
          { title: 'PERF-01 — Dashboard web+mobile: carga plan COMPLETO con todas las semanas/sesiones/logs cuando solo necesita 1 semana', done: false, priority: 'P1', note: 'Archivos: app/(athlete)/dashboard/page.tsx líneas 142-150 + api/mobile/dashboard/route.ts líneas 24-33. include: { weeks: { include: { sessions: { include: { log } } } } } sin filtro. Plan de 12 semanas = ~84 sesiones + logs cargados en cada visita. Fix: añadir where: { weekNumber: currentWeekNum } al include de weeks.' },
          { title: 'PERF-02 — week-sessions mobile: carga plan COMPLETO otra vez para extraer 1 semana — redundante con dashboard', done: false, priority: 'P1', note: 'Archivo: api/mobile/dashboard/week-sessions/route.ts líneas 27-41. Si la app llama dashboard + week-sessions al montar = 2 cargas completas del plan. Fix: filtrar directamente en la query Prisma con weeks: { where: { weekNumber: selectedWeekNum } }.' },
          { title: 'PERF-03 — N+1 gym/routines POST+PATCH: loop de creates serializados (6 días × 8 ejercicios = 54 queries) dentro de $transaction', done: false, priority: 'P1', note: 'Archivos: api/athlete/gym/routines/route.ts líneas 118-149 + api/athlete/gym/routines/[id]/route.ts líneas 61-111. Loop anidado for(days) { tx.workoutDay.create() + for(exercises) { tx.workoutExercise.create() } }. Fix: reemplazar el loop de ejercicios con createMany({ data: exercises.map(...) }).' },
          { title: 'PERF-04 — gym/session/complete: 322 LOC de lógica inline — PR detection, progression, 3 paths, setLogs × 3 bloques duplicados', done: false, priority: 'P1', note: 'Archivo: api/gym/session/complete/route.ts. PR detection (líneas 81-147), progression (150-169), free-session PR (172-191), 3 paths con createSetLogs duplicado 3 veces. Viola la regla de 40 líneas. Fix: extraer a domain/gym/complete-gym-session.use-case.ts.' },
          { title: 'PERF-05 — Dashboard web 873 LOC: lógica de getDashboardSummary duplicada inline — mobile ya usa el use case del dominio', done: false, priority: 'P1', note: 'Archivo: app/(athlete)/dashboard/page.tsx. ~500 líneas de lógica antes del JSX duplicando domain/dashboard/get-dashboard-summary.use-case.ts (buildWeeklySummary, formStatus, streak, weightProgress, volumeDelta, adherencia). Fix: refactorizar para llamar getDashboardSummary() como hace el mobile.' },
        ...
        items: [
          {
            ...
            priority: 'P1',
            note: 'Decisión de producto (2026-07-03): el sistema NO genera planes automáticamente. Modelo definitivo → B2B: el coach genera y asigna el plan. B2C Free: tracking libre (log, nutrición, gym) sin plan generado. B2C Pro (futuro): AI genera el plan + módulo de notificaciones hace el seguimiento. Cuando el plan de un atleta termine, sigue en modo tracking hasta que el coach le asigne uno nuevo o AI genere el siguiente. Impacto técnico: (1) eliminar /new-goal de la navegación y sidebar — ruta puede quedar pero sin acceso visible. (2) /api/plan/new activa featureCheckin incorrectamente — checkin es gate de Pro, no debe activarse aquí. (3) generatedBy:COACH en /api/plan/new es semánticamente incorrecto — el plan lo genera el atleta, no el coach. (4) GENERAL_FITNESS en el selector no está en el scope actual. (5) generate-plan.use-case.ts queda como legacy — solo lo usa el coach desde /coach/athlete/[id]/plan. Planes existentes en DB intactos. Archivos: src/app/(athlete)/new-goal/ · src/app/api/plan/new/route.ts · src/domain/plan/generate-plan.use-case.ts.',
          },
          {
