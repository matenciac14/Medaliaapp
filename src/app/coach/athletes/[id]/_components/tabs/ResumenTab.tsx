        ...
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border" style={{ backgroundColor: '#fffbeb', borderColor: '#fde68a' }}>
          ...
          <div className="flex-1 flex items-center gap-2 flex-wrap text-sm">
            {classifiedAlerts.map((alert, i) => (
              <Fragment key={i}>
                {i > 0 && <span className="text-amber-300 mx-1">&middot;</span>}
                <span
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            {injuryCount > 0 && (
              <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-red-50 text-red-600">
                &#9650; {injuryCount} lesi{injuryCount === 1 ? '\u00f3n activa' : 'ones activas'}
              </span>
            )}
          </div>
          ...
          <div className="space-y-3 flex-1">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">
                <span className="mr-1">&#128274;</span>Notas privadas (el atleta no las ve)
              </label>
              <textarea
                rows={2}
                value={privateNotes}
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {healthProfile.injuries.map((injury, i) => (
                      <li key={i} className="text-sm text-gray-700">&bull; {injury}</li>
                    ))}
                  </ul>
                </div>
              )}
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {healthProfile.conditions.map((cond, i) => (
                      <li key={i} className="text-sm text-gray-700">&bull; {cond}</li>
                    ))}
                  </ul>
                </div>
              )}
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-baseline gap-2 mb-3">
                ...
                <span className="text-[11px] text-gray-400">
                  Karvonen (FC rep. {healthProfile?.hrResting ?? '\u2014'})
                </span>
              </div>
              {/* Zone bars */}
            ...
            {activePlan ? (
              <>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {activePlan.totalWeeks} semanas &middot; Semana {currentWeekNum}
                </p>
              </>
            ) : (
          ...
          {dailyLogs.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <TrendItem
                  label="Peso"
                  value={weights.length >= 2 ? `${weights[0]}\u2192${weights[weights.length - 1]} kg` : weights.length === 1 ? `${weights[0]} kg` : null}
                  t={trend(weights)}
                  upIsGood={false}
                />
                <TrendItem
                  label="Energ\u00eda"
                  value={avg(energies) != null ? `${avg(energies)!.toFixed(1)} avg` : null}
                  t={trend(energies)}
                  upIsGood={true}
                />
                <TrendItem
                  ...
                  value={isRunning
                    ? (avg(hrs) != null ? `${Math.round(avg(hrs)!)} avg` : null)
                    : null
                  }
                  t={isRunning ? trend(hrs) : null}
                  upIsGood={!isRunning}
                />
                <TrendItem
                  label="Sue\u00f1o"
                  value={avg(sleeps) != null ? `${avg(sleeps)!.toFixed(1)}h avg` : null}
                  t={trend(sleeps)}
                  upIsGood={true}
                />
              </div>
