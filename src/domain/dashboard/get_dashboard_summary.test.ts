 ...
 *
 * Cómo correr:
 *   pnpm test src/domain/dashboard/get_dashboard_summary.test.ts
 */
import { describe, it, expect } from 'vitest'
import { getDashboardSummary, type DashboardInput, type PlanWeek } from './get_dashboard_summary.use_case'
...
describe('completedCount y totalTraining', () => {
  ...
  it('totalTraining excluye sesiones DESCANSO', () => {
    const { summary } = getDashboardSummary(baseInput({
      activePlanRaw: makePlanWithSessions([
        makeSession(1, 'RODAJE_Z2'),
        makeSession(3, 'DESCANSO'),
        makeSession(5, 'TEMPO'),
        makeSession(7, 'DESCANSO'),
      ]),
    }))
    expect(summary.totalTraining).toBe(2)
  })

  it('completedCount excluye sesiones DESCANSO con log', () => {
    const { summary } = getDashboardSummary(baseInput({
      activePlanRaw: makePlanWithSessions([
        makeSession(1, 'RODAJE_Z2', { log: { id: 'log-1' } }),
        makeSession(3, 'DESCANSO', { log: { id: 'log-2' } }),
        makeSession(5, 'TEMPO'),
      ]),
    }))
