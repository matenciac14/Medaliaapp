...
/** Objective training data from the week — computed from SessionLog + GymSession. */
export type WeekActivitySummary = {
  totalSessions: number          // sessions completed this week (sport + gym + free)
  totalMinutes: number           // aggregate duration in minutes
  avgRpe: number | null          // average RPE across all logged sessions (null = no RPE data)
  maxRpe: number | null          // highest RPE this week
  consecutiveActiveDays: number  // streak of days with activity ending today (or last active day)
  avgHrReal: number | null       // average HR across sessions with hrAvg data
  prevWeeksAvgMinutes: number | null  // average weekly minutes over previous 4 weeks (null = no data)
}
...
export type PlanContext = {
  ...
  weekActivity?: WeekActivitySummary  // objective training data — enriches subjective check-in
}
