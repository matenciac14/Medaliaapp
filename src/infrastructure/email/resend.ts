...
export async function sendCoachCheckInEmail(
  to: string,
  coachName: string,
  athleteName: string,
  athleteId: string,
  data: { energyLevel?: number | null; hardestRpe?: number | null; weightKg?: number | null }
) {
  const resend = new Resend(process.env.RESEND_API_KEY)
  const metrics = [
    data.energyLevel  != null ? `Energía: <strong>${data.energyLevel}/10</strong>` : null,
    data.hardestRpe   != null ? `RPE más alto: <strong>${data.hardestRpe}/10</strong>` : null,
    data.weightKg     != null ? `Peso: <strong>${data.weightKg} kg</strong>` : null,
  ].filter(Boolean).join(' · ')
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: `${athleteName} completó su check-in semanal — Medaliq`,
    html: `
<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"></head>
...
export async function sendReengagementEmail(to: string, name: string, daysSinceLastActivity: number) {
  const resend = new Resend(process.env.RESEND_API_KEY)
  await resend.emails.send({
    from: FROM,
    to,
    subject: 'Tu cuerpo te está esperando — Medaliq',
    html: `
<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"></head>
      ...
      <table width="100%" style="max-width:480px;background:white;border-radius:16px;overflow:hidden">
        ...
        <tr>
          <td style="padding:40px">
            <h1 style="margin:0 0 8px;font-size:20px;font-weight:700;color:#1e3a5f">Hola ${name} 💪</h1>
            <p style="margin:0 0 24px;font-size:14px;color:#64748b;line-height:1.6">
              Llevas <strong>${daysSinceLastActivity} días</strong> sin registrar actividad. Tu plan sigue ahí, listo para cuando tú lo estés.
            </p>
            <p style="margin:0 0 24px;font-size:14px;color:#64748b;line-height:1.6">
              No necesitas hacer una sesión perfecta — con retomar el hábito es suficiente.
            </p>
            <a href="https://medaliq.com/dashboard" style="display:inline-block;background:#ea580c;color:white;font-size:15px;font-weight:700;text-decoration:none;border-radius:12px;padding:14px 28px">
              Volver a entrenar →
            </a>
            <p style="margin:24px 0 0;font-size:12px;color:#94a3b8;line-height:1.6">
              Si prefieres no recibir estos recordatorios, escríbenos a hola@medaliq.com
            </p>
          </td>
        </tr>
        <tr>
...
export async function sendBillingRenewalReminderEmail(
  ...
  const resend = new Resend(process.env.RESEND_API_KEY)
  const dateStr = renewalDate.toLocaleDateString('es-CO', { year: 'numeric', month: 'long', day: 'numeric' })

  await resend.emails.send({
    from: FROM,
    to,
    subject: `Tu plan ${tierName} vence en 7 días — Renueva para no perder el acceso`,
    html: `
<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"></head>
      ...
      <table width="100%" style="max-width:480px;background:white;border-radius:16px;overflow:hidden">
        ...
        <tr>
          <td style="padding:40px">
            <h1 style="margin:0 0 8px;font-size:20px;font-weight:700;color:#1e3a5f">Hola ${name} 👋</h1>
            <p style="margin:0 0 16px;font-size:14px;color:#64748b;line-height:1.6">
              Tu plan <strong>${tierName}</strong> vence el <strong>${dateStr}</strong>.
            </p>
            <p style="margin:0 0 24px;font-size:14px;color:#374151;background:#fff7ed;border-left:3px solid #ea580c;border-radius:6px;padding:14px 18px;line-height:1.6">
              Renueva antes de esa fecha para mantener el acceso sin interrupciones. Después de vencer tienes 3 días de gracia antes de que tu cuenta baje de tier.
            </p>
            <a href="${renewalLink}" style="display:inline-block;background:#ea580c;color:white;font-size:15px;font-weight:700;text-decoration:none;border-radius:12px;padding:14px 28px">
              Renovar mi plan →
            </a>
            <p style="margin:24px 0 0;font-size:12px;color:#94a3b8;line-height:1.6">
              Si tienes dudas escríbenos a hola@medaliq.com
            </p>
          </td>
        </tr>
        <tr>
